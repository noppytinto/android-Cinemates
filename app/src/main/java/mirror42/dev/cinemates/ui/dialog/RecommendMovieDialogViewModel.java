...
public class RecommendMovieDialogViewModel extends ViewModel {
    ...
    //----------------------------------------------------------------------------------------- CONSTRUCTORS
    public RecommendMovieDialogViewModel() {
...
//                        postDownloadStatus(DownloadStatus.FAILED_OR_EMPTY);
    ...
    public void recommendMovie(int movieId, User selectedUser, User loggedUser) {
        final OkHttpClient httpClient = OkHttpSingleton.getClient();
        try {
            // generating url request
            final String dbFunction = "fn_recommend_movie";
            HttpUrl httpUrl = HttpUtilities.buildHttpURL(dbFunction);
            RequestBody requestBody = new FormBody.Builder()
                    .add("movie_id", String.valueOf(movieId))
                    .add("sender_email", loggedUser.getEmail())
...
}// end RecommendMovieViewModel class